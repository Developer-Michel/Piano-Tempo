...
import teacher1Image from '@assets/stock_images/professional_female__64673c14.jpg';
...
export function Teachers() {
  ...
  return (
    <section id="teachers" ref={sectionRef} className="py-24 bg-gray-50" data-testid="section-teachers">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="font-display text-4xl md:text-5xl text-black mb-4" data-testid="text-teachers-title">
            {translations.teachers.title[language]}
          </h2>
          <p className="font-serif text-xl text-gold italic" data-testid="text-teachers-subtitle">
...
function TeacherCard({
  ...
  return (
    ...
    >
      ...
      <div className="p-8">
        <h3 className="font-display text-2xl text-black mb-1" data-testid={`text-teacher-name-${index}`}>
          {teacher.name}
        </h3>
        <p className="font-sans text-gold mb-4" data-testid={`text-teacher-title-${index}`}>
          {teacher.title}
        </p>
        ...
        <div className="space-y-4">
          ...
          >
            <p className="font-sans text-gray-700 leading-relaxed whitespace-pre-line" data-testid={`text-teacher-bio-${index}`}>
          ...
          >
            {isExpanded ? (
              <>
                {translations.teachers.readLess[language]} <ChevronUp className="ml-1 w-4 h-4" />
              </>
            ) : (
              <>
                {translations.teachers.readMore[language]} <ChevronDown className="ml-1 w-4 h-4" />
              </>
            )}
          </Button>
        </div>
